<?php
$pageTitle = 'Dashboard';
$pageScripts = ['photos', 'hydrants', 'map'];
include __DIR__ . '/includes/head.php';
?>

<?php include __DIR__ . '/includes/header.php'; ?>

    <!-- Container -->
    <div class="container">
        <!-- Message Container -->
        <div id="messageContainer"></div>

        <!-- Stats Grid -->
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-icon">🔥</div>
                <div class="stat-content">
                    <div class="stat-label">Gesamt</div>
                    <div class="stat-value" id="statTotal">0</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon">📍</div>
                <div class="stat-content">
                    <div class="stat-label">Typen</div>
                    <div class="stat-value" id="statTypes">-</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon">🗺️</div>
                <div class="stat-content">
                    <div class="stat-label">Auf Karte</div>
                    <div class="stat-value" id="statVisible">0</div>
                </div>
            </div>
        </div>

        <!-- Toolbar -->
        <div class="toolbar">
            <div style="display: flex; gap: 10px; align-items: center; flex: 1;">
                <input type="text" id="searchInput" placeholder="🔍 Suchen..." style="padding: 8px 12px; border: 1px solid #ddd; border-radius: 4px; flex: 1; max-width: 400px;">
            </div>
            <button class="btn-primary" id="newHydrantBtn">+ Neuer Hydrant</button>
        </div>

        <!-- Map -->
        <div class="map-container">
            <div id="map"></div>
        </div>

        <!-- Mobile Sort Controls -->
        <div class="mobile-sort-controls">
            <label>Sortieren:</label>
            <select id="mobileSortField">
                <option value="type">Typ</option>
                <option value="title">Titel</option>
                <option value="description">Beschreibung</option>
                <option value="lat">Koordinaten</option>
                <option value="updated_at">Geändert</option>
            </select>
            <button id="mobileSortToggle" title="Sortierreihenfolge umkehren">↕️</button>
        </div>

        <!-- Table -->
        <div class="table-container">
            <table class="hydrant-table">
                <thead>
                    <tr>
                        <th style="width: 80px" data-sort="type">Typ <span class="sort-icon">⇅</span></th>
                        <th data-sort="title">Titel <span class="sort-icon">⇅</span></th>
                        <th data-sort="description">Beschreibung <span class="sort-icon">⇅</span></th>
                        <th style="width: 150px" data-sort="lat">Koordinaten <span class="sort-icon">⇅</span></th>
                        <th style="width: 120px" data-sort="updated_at">Geändert <span class="sort-icon">⇅</span></th>
                        <th style="width: 220px">Aktionen</th>
                    </tr>
                </thead>
                <tbody id="hydrantTableBody">
                    <tr>
                        <td colspan="6">
                            <div class="loading">
                                <div class="spinner"></div>
                                <div>Lade Hydranten...</div>
                            </div>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>

    <!-- Modal: Hydrant bearbeiten/erstellen -->
    <div class="modal-overlay" id="hydrantModal">
        <div class="modal">
            <div class="modal-header">
                <h2 class="modal-title" id="modalTitle">Hydrant bearbeiten</h2>
                <button class="modal-close" id="closeModal">&times;</button>
            </div>
            <form id="hydrantForm">
                <div class="modal-body">
                    <input type="hidden" name="id" id="id_field">
                    
                    <!-- Karte zur Positionsauswahl -->
                    <div class="form-group">
                        <label>Position auf Karte wählen</label>
                        <div style="position: relative;">
                            <div id="modalMap" class="modal-map"></div>
                            <button type="button" id="modalGpsButton" class="modal-gps-button" title="Genaue GPS-Position verwenden" aria-label="GPS-Position">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <circle cx="12" cy="12" r="10"/>
                                    <circle cx="12" cy="12" r="3"/>
                                    <line x1="12" y1="1" x2="12" y2="5"/>
                                    <line x1="12" y1="19" x2="12" y2="23"/>
                                    <line x1="1" y1="12" x2="5" y2="12"/>
                                    <line x1="19" y1="12" x2="23" y2="12"/>
                                </svg>
                            </button>
                        </div>
                        <small>Klicke auf die Karte oder ziehe den Marker um die Position zu ändern. GPS-Button für genaue Position.</small>
                    </div>

                    <div class="form-row">
                        <div class="form-group">
                            <label for="type">Typ *</label>
                            <select name="type" id="type" required>
                                <option value="">Lade Typen...</option>
                            </select>
                        </div>

                        <div class="form-group">
                            <label for="title">Titel *</label>
                            <input type="text" name="title" id="title" required placeholder="z.B. Anwesen Bernhardt">
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="description">Beschreibung</label>
                        <textarea name="description" id="description" placeholder="z.B. An der Zaunecke"></textarea>
                    </div>

                    <div class="form-row">
                        <div class="form-group">
                            <label for="lat">Breitengrad *</label>
                            <input type="number" name="lat" id="lat" step="0.000001" required placeholder="50.000000">
                        </div>

                        <div class="form-group">
                            <label for="lng">Längengrad *</label>
                            <input type="number" name="lng" id="lng" step="0.000001" required placeholder="7.360000">
                        </div>
                    </div>

                    <!-- Foto-Galerie -->
                    <div class="form-group">
                        <label>📸 Fotos</label>
                        <div id="photoGallery" class="photo-gallery"></div>
                        <div class="photo-upload">
                            <input type="file" id="photoUpload" accept="image/*" multiple style="display:none">
                            <input type="file" id="photoCameraUpload" accept="image/*" capture="environment" style="display:none">
                            <button type="button" class="btn-secondary" id="photoUploadBtn">🖼️ Aus Galerie</button>
                            <button type="button" class="btn-secondary" id="photoCameraBtn">📷 Kamera</button>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn-action" onclick="Hydrants.closeModal()">Abbrechen</button>
                    <button type="submit" class="btn-primary">Speichern</button>
                </div>
            </form>
        </div>
    </div>

    <?php include __DIR__ . '/includes/scripts.php'; ?>

    <!-- Page-specific Init -->
    <script>
        // App initialisieren
        async function initApp() {
            // Session prüfen
            const isLoggedIn = await Auth.checkSession();

            if (!isLoggedIn) {
                Auth.requireLogin();
                return;
            }

            // User-Info anzeigen
            Auth.displayUserInfo();

            // UI für Rolle anpassen
            Auth.updateUIForRole();

            // Menu generieren (nach Auth-Check!)
            AdminMenu.generate();
            
            // Event-Listener
            setupLogout();
            setupNewHydrantButton();
            setupHydrantForm();
            setupModalClose();
            setupSearch();
            setupTableSort();
            setupMobileSort();
            setupTableScrollIndicator();
            setupPasswordChange();
            setupPhotoUpload();
            setupReloadButton();

            // Debug-Einstellungen laden (Reload-Button anzeigen)
            applyDebugSettings();

            // Karte initialisieren
            await AdminMap.initMainMap();

            // Marker-Typen laden (vor Hydranten!)
            await Hydrants.loadMarkerTypes();

            // Hydranten laden
            await Hydrants.loadAll();
        }
        
        // App starten wenn DOM bereit
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initApp);
        } else {
            initApp();
        }
    </script>
    
    <!-- Migration Script -->
    <script>
    async function runMigration() {
        if (!confirm('Alte Fotos zu neuem System migrieren?\n\nDies kopiert alle Fotos aus /pictures_app/ nach /uploads/')) return;
        
        try {
            showMessage('Migration läuft...', 'info');
            
            const response = await fetch('../api/migrate-photos.php', {
                credentials: 'include'
            });
            const result = await response.json();
            
            if (result.success) {
                showMessage(`Migration erfolgreich! ${result.data.migrated} Fotos migriert.`, 'success');
                document.getElementById('migrationBanner').style.display = 'none';
                
                // Hydranten neu laden
                setTimeout(() => {
                    Hydrants.loadAll();
                }, 1000);
            } else {
                throw new Error(result.error || 'Migration fehlgeschlagen');
            }
        } catch (error) {
            console.error('Migration-Fehler:', error);
            showMessage('Fehler: ' + error.message, 'error');
        }
    }
    </script>

<!-- Change Password Modal -->
<div class="modal-overlay" id="changePasswordModal">
    <div class="modal">
        <div class="modal-header">
            <h2 class="modal-title">Passwort ändern</h2>
            <button class="modal-close" id="closeChangePassword">&times;</button>
        </div>
        <form id="changePasswordForm">
            <div class="modal-body">
                <!-- Hidden username field for accessibility -->
                <input type="text" name="username" autocomplete="username" style="display: none;" aria-hidden="true" tabindex="-1">
                
                <div id="passwordChangeNotice" class="alert alert-warning" style="display: none;">
                    <strong>⚠️ Passwort muss geändert werden</strong>
                    <p>Aus Sicherheitsgründen müssen Sie Ihr Passwort bei der ersten Anmeldung ändern.</p>
                </div>
                
                <div class="form-group">
                    <label for="currentPassword">Aktuelles Passwort *</label>
                    <input type="password" name="current_password" id="currentPassword" required 
                           autocomplete="current-password">
                </div>
                
                <div class="form-group">
                    <label for="newPassword">Neues Passwort * <small>(mind. 8 Zeichen)</small></label>
                    <input type="password" name="new_password" id="newPassword" required 
                           minlength="8" autocomplete="new-password">
                </div>
                
                <div class="form-group">
                    <label for="confirmPassword">Neues Passwort bestätigen *</label>
                    <input type="password" name="confirm_password" id="confirmPassword" required 
                           minlength="8" autocomplete="new-password">
                </div>
                
                <div class="password-strength" id="passwordStrength"></div>
            </div>
            
            <div class="modal-footer">
                <button type="button" class="btn-secondary" id="cancelPasswordChange">Abbrechen</button>
                <button type="submit" class="btn-primary">Passwort ändern</button>
            </div>
        </form>
    </div>
</div>

<!-- Back to Top Button -->
<button id="backToTop" class="back-to-top" aria-label="Nach oben" title="Nach oben">↑</button>

<!-- New Hydrant FAB -->
<button id="newHydrantFAB" class="new-hydrant-fab" aria-label="Neuer Hydrant" title="Neuer Hydrant">+</button>

<!-- Install Prompt (nur wenn noch nicht installiert) -->
<div id="installPrompt" class="install-prompt" style="display: none;">
    <p>LoeschNetz Admin als App installieren?</p>
    <div class="install-actions">
        <button id="installButton">Installieren</button>
        <button id="dismissInstall">Später</button>
    </div>
</div>

<!-- Service Worker Registration -->
<script>
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('./sw.js')
            .then(registration => {
                console.log('✅ Admin SW registered:', registration.scope);

                // Check for updates
                registration.addEventListener('updatefound', () => {
                    const newWorker = registration.installing;
                    console.log('🔄 Admin SW update found');

                    newWorker.addEventListener('statechange', () => {
                        if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                            console.log('✨ Admin SW update available - reload to activate');
                            // Optionally show update notification
                        }
                    });
                });
            })
            .catch(error => {
                console.error('❌ Admin SW registration failed:', error);
            });
    });
}

// PWA Install Prompt
let deferredPrompt;
let installPromptShown = false;

// Verstecke Install-Button wenn App bereits installiert ist
function checkInstallStatus() {
    const installBtn = document.getElementById('installAppBtn');
    if (!installBtn) return;

    // 1. Prüfe ob im Standalone-Modus (PWA läuft)
    if (window.matchMedia('(display-mode: standalone)').matches) {
        installBtn.style.display = 'none';
        console.log('✅ App läuft im Standalone-Modus, Install-Button versteckt');
        return true;
    }

    // 2. Prüfe localStorage (wurde bereits installiert?)
    const isInstalled = localStorage.getItem('pwaInstalled');
    if (isInstalled === 'true') {
        installBtn.style.display = 'none';
        console.log('✅ PWA bereits installiert (localStorage), Install-Button versteckt');
        return true;
    }

    // 3. Prüfe mit getInstalledRelatedApps API (Chrome/Edge)
    if ('getInstalledRelatedApps' in navigator) {
        navigator.getInstalledRelatedApps().then(relatedApps => {
            if (relatedApps.length > 0) {
                installBtn.style.display = 'none';
                localStorage.setItem('pwaInstalled', 'true');
                console.log('✅ PWA bereits installiert (getInstalledRelatedApps), Install-Button versteckt');
            }
        }).catch(err => {
            console.log('ℹ️ getInstalledRelatedApps nicht verfügbar:', err.message);
        });
    }

    return false;
}

// Prüfe beim Laden
checkInstallStatus();

// Debug-Modus: URL-Parameter ?resetpwa=1 löscht localStorage
const urlParams = new URLSearchParams(window.location.search);
if (urlParams.get('resetpwa') === '1') {
    localStorage.removeItem('hasSeenInstallPrompt');
    localStorage.removeItem('pwaInstalled');
    console.log('🔄 Install-Prompt Status zurückgesetzt');
    // Zeige visuelle Bestätigung
    setTimeout(() => {
        if (window.showMessage) {
            showMessage('Install-Prompt zurückgesetzt! Seite wird neu geladen...', 'success');
        }
        // Entferne Parameter und lade neu
        window.location.href = window.location.pathname;
    }, 1000);
}

window.addEventListener('beforeinstallprompt', (e) => {
    console.log('📱 beforeinstallprompt Event gefeuert');
    e.preventDefault();
    deferredPrompt = e;

    // Zeige Install-Button im Header
    const installBtn = document.getElementById('installAppBtn');
    if (installBtn && !window.matchMedia('(display-mode: standalone)').matches) {
        installBtn.style.display = 'inline-block';
        console.log('✅ Install-Button im Header angezeigt');
    }

    // Prüfe ob bereits angezeigt wurde
    const hasSeenInstallPrompt = localStorage.getItem('hasSeenInstallPrompt');

    // Zeige Install-Prompt nur beim ersten Mal automatisch (nach 3 Sekunden)
    if (!hasSeenInstallPrompt && !installPromptShown) {
        setTimeout(() => {
            const installPrompt = document.getElementById('installPrompt');
            if (installPrompt && !window.matchMedia('(display-mode: standalone)').matches) {
                installPrompt.style.display = 'block';
                installPromptShown = true;
                console.log('✅ Automatischer Install-Prompt angezeigt');
            }
        }, 3000);
    }
});

// Fallback für Browser ohne beforeinstallprompt (z.B. Firefox)
// Zeige Button trotzdem an, wenn PWA-Kriterien erfüllt sind
setTimeout(() => {
    if (!deferredPrompt && !window.matchMedia('(display-mode: standalone)').matches) {
        // Prüfe ob es ein Service Worker gibt (PWA-Kriterium)
        if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
            const installBtn = document.getElementById('installAppBtn');
            if (installBtn) {
                installBtn.style.display = 'inline-block';
                installBtn.title = 'App installieren (über Browser-Menü)';
                console.log('ℹ️ Fallback: Install-Button angezeigt (kein beforeinstallprompt Event)');
            }
        }
    }
}, 2000);

// Funktion zum manuellen Anzeigen des Install-Prompts
window.showInstallPrompt = function() {
    const installPrompt = document.getElementById('installPrompt');

    if (deferredPrompt && installPrompt && !window.matchMedia('(display-mode: standalone)').matches) {
        // Chrome/Edge/Vivaldi: Nutze Standard-Prompt
        installPrompt.style.display = 'block';
    } else if (window.matchMedia('(display-mode: standalone)').matches) {
        alert('Die App ist bereits installiert!');
    } else {
        // Firefox oder anderer Browser ohne beforeinstallprompt
        alert('Bitte nutzen Sie das Browser-Menü, um die App zu installieren:\n\n' +
              '• Firefox: Menü → "Diese Seite installieren"\n' +
              '• Chrome/Edge: Menü → "App installieren"\n' +
              '• Safari: Teilen → "Zum Home-Bildschirm"');
    }
};

// Funktion zum Ausblenden des Install-Prompts mit Animation
function hideInstallPrompt() {
    const installPrompt = document.getElementById('installPrompt');
    if (installPrompt) {
        installPrompt.classList.add('slide-out');
        setTimeout(() => {
            installPrompt.style.display = 'none';
            installPrompt.classList.remove('slide-out');
        }, 300);
    }
}

// Install-Button Handler
document.getElementById('installButton')?.addEventListener('click', async () => {
    if (deferredPrompt) {
        deferredPrompt.prompt();
        const { outcome } = await deferredPrompt.userChoice;
        console.log('Install outcome:', outcome);

        // Markiere als gesehen
        localStorage.setItem('hasSeenInstallPrompt', 'true');

        // Verstecke Popup mit Animation
        hideInstallPrompt();

        // Verstecke Header-Button nach Installation
        if (outcome === 'accepted') {
            // Markiere PWA als installiert
            localStorage.setItem('pwaInstalled', 'true');

            const installBtn = document.getElementById('installAppBtn');
            if (installBtn) {
                installBtn.style.display = 'none';
            }

            console.log('✅ PWA installiert, Status gespeichert');
        }

        deferredPrompt = null;
    }
});

// Dismiss-Button Handler
document.getElementById('dismissInstall')?.addEventListener('click', () => {
    // Markiere als gesehen
    localStorage.setItem('hasSeenInstallPrompt', 'true');
    hideInstallPrompt();
});
</script>

<!-- Back to Top Button Functionality -->
<script>
(function() {
    const backToTopButton = document.getElementById('backToTop');
    const scrollThreshold = 300;

    function toggleBackToTopButton() {
        if (window.pageYOffset > scrollThreshold || document.documentElement.scrollTop > scrollThreshold) {
            backToTopButton.classList.add('visible');
        } else {
            backToTopButton.classList.remove('visible');
        }
    }

    function scrollToTop() {
        window.scrollTo({
            top: 0,
            behavior: 'smooth'
        });
    }

    window.addEventListener('scroll', toggleBackToTopButton);
    backToTopButton.addEventListener('click', scrollToTop);

    toggleBackToTopButton();
})();
</script>

<!-- New Hydrant FAB Functionality -->
<script>
(function() {
    const newHydrantFAB = document.getElementById('newHydrantFAB');
    const scrollThreshold = 300;

    function toggleNewHydrantFAB() {
        if (window.pageYOffset > scrollThreshold || document.documentElement.scrollTop > scrollThreshold) {
            newHydrantFAB.classList.add('visible');
        } else {
            newHydrantFAB.classList.remove('visible');
        }
    }

    function openNewHydrantModal() {
        if (window.Hydrants && typeof window.Hydrants.createNew === 'function') {
            window.Hydrants.createNew();
        }
    }

    window.addEventListener('scroll', toggleNewHydrantFAB);
    newHydrantFAB.addEventListener('click', openNewHydrantModal);

    toggleNewHydrantFAB();
})();
</script>

<?php include __DIR__ . '/includes/footer.php'; ?>
